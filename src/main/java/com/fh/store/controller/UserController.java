...
public class UserController extends BaseController{
    ...
    /*
    @RequestMapping("reg")
    //@ResponseBody//表示此方法 的响应结果以json格式进行数据的响应给到前端
    public JsonResult<Void> reg(User user){
        ...
        } catch (UsernameDuplicatedException exception) {
    ...
    /**
     * 约定大于配置：开发思想来完成，省略大量的配置来编写。
     * 1.接受数据方式：请求处理方法的参数列表设置为pojo类型来接受前端的数据
     *      SpringBoot会将前端的url地址中的参数名和pojo类的属性名进行比较，如果这两个名称项目则将值注入到pojo类中对应的属性上
     */
    @RequestMapping("reg")
    //@ResponseBody//表示此方法 的响应结果以json格式进行数据的响应给到前端
    public JsonResult<Void> reg(User user){
    ...
    /**
     * 2.接受数据方式：请求处理方法的参数列表设置为非pojo类型，直接将SpringBoot请求的参数名和方法的参数名直接进行比较
     * ，如果名称相同则自动完成值的注入
     */
